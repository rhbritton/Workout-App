		...
		if(!$scope.exercising) {
			pauseExercise = $interval(function() {
				...
				var timeLeft = $scope.exercise.amount - $scope.time
				  , evenTime = $scope.exercise.amount % 2 ? ($scope.exercise.amount + 1) : $scope.exercise.amount
				  , isHalfTime = evenTime/2 == $scope.time

				console.log(evenTime/2)

				console.log($scope.time, isHalfTime)
